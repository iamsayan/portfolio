 ...
 * Optimized for Astro and standard fetch runtimes.
 */
...
export interface CockpitConfig {
  ...
  apiKey: string;
  /** Custom options for fetch requests (e.g. headers, credentials configurations) */
...
export type CollectionItem<T extends object = Record<string, unknown>> = (Entity | TreeEntity) & T;
...
export class CockpitClient {
  ...
  private async request<T = unknown>(
    ...
    if (!response.ok) {
      ...
      throw new Error(`Cockpit request failed with status ${response.status}: ${errorText}`);
    }
   ...
   */
  async listAssets(options: ListAssetsOptions = {}, fetchOptions?: RequestInit): Promise<Asset[]> {
    return this.request<Asset[]>(`/assets${this.buildQuery(options)}`, "GET", undefined, undefined, fetchOptions);
  }
  ...
  async getAsset(id: string, fetchOptions?: RequestInit): Promise<Asset> {
    return this.request<Asset>(`/assets/${id}`, "GET", undefined, undefined, fetchOptions);
  }
      ...
      Object.entries(meta).forEach(([key, value]) => {
        if (value !== undefined && value !== null) {
          if (Array.isArray(value)) {
            value.forEach((val) => {
              if (val !== undefined && val !== null) {
                formData.append(`meta[${key}][]`, typeof val === "object" ? JSON.stringify(val) : String(val));
              }
            });
   ...
   */
  async updateAsset(asset: { _id: string; [key: string]: unknown }): Promise<Asset> {
   ...
   */
  getImagePresetUrl(id: string, preset: string, options: ImagePresetOptions = {}): string {
   ...
   */
  async listPresets(fetchOptions?: RequestInit): Promise<Record<string, Preset>> {
    return this.request<Record<string, Preset>>("/assets/presets", "GET", undefined, undefined, fetchOptions);
  }
   ...
   */
  async saveContentItem<T = CollectionItem>(model: string, data: Partial<T> & { _id?: string }): Promise<T> {
   ...
   */
  async deleteContentItem<T = { success: boolean }>(model: string, id: string): Promise<T> {
   ...
   */
  async batchListContentItems<T = Record<string, { entries: CollectionItem[] }>>(
    models: ContentItemsBatchModels,
    options: ContentItemsBatchQueryOptions = {},
    fetchOptions?: RequestInit
  ): Promise<T> {
    const params = { models, ...options };
    return this.request<T>(`/content/items${this.buildQuery(params)}`, "GET", undefined, undefined, fetchOptions);
  }
  ...
  async submitInbox<T = InboxSubmitResponse>(
    ...
    return this.request<T>(`/inbox/submit/${token}${this.buildQuery(options)}`, "POST", { data });
  }
  ...
  async healthcheck<T = unknown>(fetchOptions?: RequestInit): Promise<T> {
    return this.request<T>("/system/healthcheck", "GET", undefined, undefined, fetchOptions);
  }
}
